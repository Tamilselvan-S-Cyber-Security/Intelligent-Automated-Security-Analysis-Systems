...
def handle_unpacking_error():
    """Display detailed error information for unpacking errors"""
    exc_type, exc_value, exc_traceback = sys.exc_info()
    if exc_type and "unpack" in str(exc_value).lower():
        st.error("🐛 **Unpacking Error Detected**")
        st.error(f"Error: {exc_value}")
        st.code(traceback.format_exc())
        st.info("💡 This error occurs when trying to assign values that don't match the expected number of variables.")
        return True
    return False

# Set page configuration
st.set_page_config(
    page_title="Intelligent Automated Security Analysis System",
    page_icon="🐺",
    layout="wide",
...
st.title("🐺 Intelligent Automated Security Analysis System")
st.markdown("An Intelligent Automated Security Analysis System for Applications, Websites, and Files")
st.markdown("Advanced Cybersecurity Analysis Platform powered by Wolf API")
st.markdown("Developed by Security Team - SKP Engineering College, CSE 4th Year | Final Year Project")
                    ...
                    with st.spinner("🔍 Analyzing..."):
                        try:
                            ...
                            with st.spinner("📊 Generating Reports..."):
                                ...
                                html_report = report_generator.generate_reports()
                                pdf_report = None
                                ...
                                with col2:
                                    if pdf_report:
                                        ...
                                        st.info("PDF report generation is not available.")
                            ...
                            # Show full traceback to pinpoint the exact location
                            st.code(traceback.format_exc())
                ...
                with clone_tab3:
                    ...
                    if not external_domains:
                        ...
                        if domains_by_type:
                            ...
                            # Set index for proper chart display
...
<body>
    ...
    # Network Scanner Section
        ...
        if not NETWORK_SCANNER_AVAILABLE:
                ...
                if target:
                    with st.spinner("Performing network analysis..."):
                        try:
                            ...
                            elif scan_type == "Subdomain Path Traversal":
                                st.subheader("Subdomain Path Traversal Results")
                                
                                with st.spinner("🔍 Scanning subdomains for path traversal vulnerabilities..."):
                                    traversal_result = scanner.subdomain_path_traversal(target)
                                
                                if traversal_result.get('status') == 'success':
                                    # Summary
                                    summary = traversal_result.get('summary', {})
                                    col1, col2, col3, col4 = st.columns(4)
                                    
                                    with col1:
                                        st.metric("Subdomains Found", summary.get('total_subdomains_found', 0))
                                    with col2:
                                        st.metric("Exposed Paths", summary.get('total_exposed_paths', 0))
                                    with col3:
                                        st.metric("Vulnerable Paths", summary.get('total_vulnerable_paths', 0))
                                    with col4:
                                    ...
                                    with col1:
                                        if traversal_result.get('subdomains'):
                                            with st.expander("🔍 Discovered Subdomains", expanded=True):
                                                for subdomain in traversal_result['subdomains']:
                                                    st.write(f"• {subdomain}")
                                        
                                        if traversal_result.get('exposed_paths'):
                                            with st.expander("⚠️ Exposed Sensitive Paths", expanded=True):
                                                for path in traversal_result['exposed_paths']:
                                                    st.write(f"• {path}")
                                    
                                    with col2:
                                        if traversal_result.get('vulnerable_paths'):
                                            with st.expander("🚨 Path Traversal Vulnerabilities", expanded=True):
                                                for vuln in traversal_result['vulnerable_paths']:
                                                    st.error(f"• {vuln}")
                                        
                                        # Scan details
                                        with st.expander("📊 Scan Details", expanded=False):
                                            st.json(traversal_result)
                                    
                                    # Download results
                                    if traversal_result.get('subdomains') or traversal_result.get('exposed_paths') or traversal_result.get('vulnerable_paths'):
                                        report_data = {
                                            'target': target,
                                            'scan_type': 'Subdomain Path Traversal',
                                            'scan_time': traversal_result.get('scan_time'),
                                            'summary': summary,
                                            'subdomains': traversal_result.get('subdomains', []),
                                            'exposed_paths': traversal_result.get('exposed_paths', []),
                                            'vulnerable_paths': traversal_result.get('vulnerable_paths', [])
                                        }
                                        
                                        st.download_button(
                                            label="📥 Download JSON Report",
                                            data=json.dumps(report_data, indent=2),
                                            file_name=f"subdomain_path_traversal_{target}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                                            mime="application/json"
                                        )
                                else:
                                    st.error(f"❌ Scan failed: {traversal_result.get('status', 'Unknown error')}")
                                    ...
                                    for test in manual_tests:
                                        with st.expander(f"Results: {test}"):
                                            if "DNS Lookup (A Record)" in test:
                                                ...
                                                st.json(result)
                                            elif "Subdomain Path Traversal" in test:
                                                result = scanner.subdomain_path_traversal(target)
                                                st.json(result)
            ...
            st.write("• **Subdomain Security:** Path traversal detection, exposed directory scanning")
